...
class CPE:
    ...
    def _getLegalMonetaryTotal21(self, invoice_id):
        ...
        sumLineExtensionAmount = float(0)
        for line in invoice_id.invoice_line_ids.filtered(lambda ln: ln.price_subtotal >= 0):
            sumLineExtensionAmount = float(sumLineExtensionAmount) + float(line.price_subtotal)

        lineExtensionAmountTag = etree.QName(self._cbc, 'LineExtensionAmount')
        lineExtensionAmount = etree.SubElement( total, lineExtensionAmountTag.text, currencyID=invoice_id.currency_id.name, nsmap={'cbc': lineExtensionAmountTag.namespace} )
    ...
    def get_tax_amount(self, invoice_id):
    ...
    def _getPaymentMeans21( self, invoice_id ):
        ...
        PaymentMeans = etree.QName(self._cac, 'PaymentMeans')
        PaymentMeansCode = etree.QName(self._cbc, 'PaymentMeansCode')
        PaymentID = etree.QName(self._cbc, 'PaymentID')

        PayeeFinancialAccount = etree.QName(self._cac, 'PayeeFinancialAccount')
        Id = etree.QName(self._cbc, 'ID')
        ...
        if( invoice_id.sunat_payment_means_id ):
            ...
            PaymentID.text = str( invoice_id.reference )

        # PayeeFinancialAccount = etree.SubElement( self._root, PayeeFinancialAccount.text, nsmap={'cac': PayeeFinancialAccount.namespace} )
        # Id = etree.SubElement( PayeeFinancialAccount, Id.text, nsmap={'cbc': Id.namespace} )
        # Id.text = str( invoice_id.account_id.code )
    ...
    def getInvoice(self, invoice_id):
        ...
        self._getPaymentMeans21(invoice_id)   
    ...
    def getSummaryDocuments(self, batch):
        try:
           ...
           log.debug("generate__")
           log.debug(getattr(e, 'message', repr(e))+" ON LINE "+format(sys.exc_info()[-1].tb_lineno))    
           return None
...
class Document(object):
    ...
    def send(self):
        try:
           ...
           exc_traceback = sys.exc_info()
           log.debug("PE_SEND")
           log.warning(getattr(e, 'message', repr(e))+" ON LINE "+format(sys.exc_info()[-1].tb_lineno))
...
def get_sign_document(xml, key_file, crt_file):
    ...
    #except Exception as e:
    #       log.warning(xml.encode('utf-8'))
    #       log.warning(BytesIO(xml.encode('utf-8')))
    #       exc_traceback = sys.exc_info()
    #       log.debug("get_sign_document")
    #       log.debug(getattr(e, 'message', repr(e))+" ON LINE "+format(sys.exc_info()[-1].tb_lineno))    
    #       return None  
