...
class PeSunatCpe(models.Model):
    ...
    def send_cpe(self):
        ...
        try:
            ...
            self.ensure_one()
            _logger.warning("self.invoice_ids[0].id")
            ...
            _logger.warning(xml.encode('utf-8'))
            _logger.warning(BytesIO(xml.encode('utf-8')))
            exc_traceback = sys.exc_info()
            _logger.debug("get_sign_document")
            _logger.debug(getattr(e, 'message', repr(e))+" ON LINE "+format(sys.exc_info()[-1].tb_lineno))    
            return None  
        return res
    ...
    def action_document_status(self):
        ...
        _logger.warning("action_document_status")
