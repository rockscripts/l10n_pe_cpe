...
class PeSunatCpe(models.Model):
    ...
    def fix(self, like_name, response_like, mes, start_day, end_day, verify=False):
        ...
        for x1 in xrange(start_day,end_day):
            ...
            _logger.warning("SEARCH CRITERIA")
            _logger.warning([("name","ilike",like_name),("response","ilike",response_like),("date","=",fecha_busqueda)])
            edocuments = self.env['pe.cpe'].search([("name","ilike",like_name),("response","ilike",response_like),("date","=",fecha_busqueda)])
            ...
            if(edocuments):
                for edocument in edocuments:
                    ...
                    # pe.cpe(512, 471, 422, 384, 339, 298, 248, 203, 160, 116)
                        ...
                        _logger.warning("FIXING LIST")
                        _logger.warning(edocument.summary_ids)         
                        for account_invoice in edocument.summary_ids:
                            summary_invoice = self.env['pe.cpe'].search([("name","=",account_invoice.number)], limit=1)
                            _logger.warning(summary_invoice)
                            if(summary_invoice):
                                    xml_document = str(summary_invoice.xml_document)
                                    xml_document = str(xml_document).replace('catalogo06">-</cbc:ID>','catalogo06">00000000</cbc:ID>')
                                    summary_invoice.update({'xml_document':xml_document})
                                    summary_invoice.sudo().action_cancel()
                                    summary_invoice.sudo().action_draft()
                                    summary_invoice.sudo().action_generate()
                                    _logger.warning("ACTIONSEND")
                                    #summary_invoice.sudo().action_send()
